...
for f in $(cat ../module-info-filtered.csv ); do
	...
	echo "\\Use{${slug}=${mod}_name} & ${slug} & ${method_count} & ${class_count} & ${order_count} & $tso_detected & $iso_detected \\\\";
...
echo "\hline \\multicolumn{2}{|l||}{\textbf{Total}} & \\textbf{${total_method_count}} & \\textbf{${total_class_count}} & \\textbf{${total_test_orders}} & \\textbf{${total_tso_detected}} & \\textbf{${total_iso_detected}} \\\\"
